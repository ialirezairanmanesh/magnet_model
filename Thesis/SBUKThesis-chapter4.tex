...
\begin{itemize}
    ...
    \item \textbf{داده‌های ترتیبی}: توالی‌های فراخوانی \lr{API} که الگوهای رفتاری زمانی در حین اجرا را منعکس می‌کنند.
\end{itemize}
این رویکرد چندوجهی، استخراج اطلاعات مکمل را تضمین می‌کند و خلأهای تحلیل‌های تک‌وجهی را پر می‌کند. افزون بر این، از معماری‌های پیشرفته یادگیری عمیق\lr{—}ترنسفورمرها و \lr{GNN} ها\lr{—}برای مدل‌سازی الگوهای پیچیده در داخل و بین این وجه‌ها استفاده شده است. ترنسفورمرها در ثبت وابستگی‌های بلندمدت در داده‌های ترتیبی و جدولی برتری دارند، در حالی که \lr{GNNها} داده‌های گرافی را با بهره‌گیری از روابط گره‌ها و یال‌ها به طور مؤثر پردازش می‌کنند \cite{attention, Kipf2017}.

نوآوری اصلی این پژوهش در مدل \textbf{\lr{MAGNET}} نهفته است که سه ماژول تخصصی \textit{\lr{EnhancedTabTransformer}}، \textit{\lr{GraphTransformer}} و \textit{\lr{SequenceTransformer}}را با یک مکانیزم توجه پویا و لایه ادغام چندوجهی ترکیب می‌کند. مکانیزم توجه پویا به طور تطبیقی اهمیت ویژگی‌ها را در هر وجه تنظیم می‌کند، در حالی که لایه ادغام، بازنمایی‌های چندوجهی را به یک فضای ویژگی منسجم برای طبقه‌بندی تبدیل می‌کند. این طراحی، دقت شناسایی، پایداری و تطبیق‌پذیری در برابر تهدیدات در حال تحول را بهبود می‌بخشد و \lr{MAGNET} را به پیشرفتی چشمگیر نسبت به روش‌های موجود تبدیل می‌کند.

\subsection{فرضیات و ابزارهای محاسباتی}

\subsubsection{فرضیات}
طراحی و ارزیابی \lr{MAGNET} بر اساس فرضیات زیر استوار است:
\begin{enumerate}
    \renewcommand{\labelitemi}{\lr{$\bullet$}}
    ...
    \item \textbf{مناسب بودن معماری‌های پیشرفته}: ترنسفورمرها و \lr{GNN}ها برای استخراج الگوهای پیچیده از داده‌های ساختاریافته و ترتیبی مناسب هستند.
    \item \textbf{کارایی توجه پویا}: مکانیزم توجه پویا با اختصاص وزن‌های آگاه از زمینه به ویژگی‌ها، عملکرد مدل را ارتقا می‌دهد.
    \item \textbf{بهینه‌سازی مؤثر}: استفاده از بهینه‌ساز \lr{Adam} برای به‌روزرسانی وزن‌ها و \lr{Optuna} برای تنظیم ابرپارامترها، همگرایی کارآمد و پیکربندی بهینه مدل را تضمین می‌کند.
\end{enumerate}

\subsubsection{ابزارهای محاسباتی}
پیاده‌سازی و ارزیابی \lr{MAGNET} با استفاده از ابزارها و منابع زیر انجام شد:
\begin{itemize}
    \renewcommand{\labelitemi}{\lr{$\bullet$}}
    ...
    \item \textbf{سخت‌افزار}:
        ...
        \end{itemize}
    \item \textbf{دیتاست}: دیتاست \lr{DREBIN} \cite{Drebin}، شامل ۵٬۵۶۰ نمونه بدافزار و ۵٬۰۰۰ نمونه سالم. این دیتاست شامل:
        \begin{itemize}
            \renewcommand{\labelitemii}{\lr{$\bullet$}}
            ...
            \item ویژگی‌های ترتیبی: توالی‌های فراخوانی \lr{API} با میانگین طول ۸۷ فراخوانی در هر نمونه.
        \end{itemize}
\end{itemize}
...
دیتاست \lr{DREBIN} برای سازگاری با مدل \lr{MAGNET} پیش‌پردازش شد. هر وجه به صورت زیر پردازش شد:
...
\begin{itemize}
    ...
    \item \textbf{داده‌های جدولی}:
        \begin{itemize}
            \renewcommand{\labelitemii}{\lr{$\bullet$}}
            \item \textit{استخراج ویژگی}: ۱۲۸ ویژگی ایستا استخراج شد، مانند تعداد مجوزها، تعداد فایل‌ها و اندازه برنامه، که یک بردار ویژگی \lr{\( \mathbf{x}_{\text{tab}} \in \mathbb{R}^{128} \)} را تشکیل می‌دهند.
    ...
    \item \textbf{داده‌های گرافی}:
        \begin{itemize}
            ...
            \item \textit{فرمت‌بندی}: گراف‌ها به اشیاء \lr{\texttt{Data}} در \lr{PyTorch Geometric} تبدیل شدند و ماتریس‌های مجاورت برای کاهش مصرف حافظه، اسپارس‌سازی شدند\LTRfootnote{میانگین اسپارسیتی = ۰.۰۰۲۵}.
        \end{itemize}
    \item \textbf{داده‌های ترتیبی}:
        \begin{itemize}
            \renewcommand{\labelitemii}{\lr{$\bullet$}}
            \item \textit{استخراج توالی}: توالی‌های فراخوانی \lr{API} به طول ثابت ۱۰۰ کوتاه یا پد شدند، که \( \mathbf{x}_{\text{seq}} \in \mathbb{Z}^{100} \) را تشکیل می‌دهند.
            \item \textit{کدگذاری}: یک واژه‌نامه با ۲٬۱۳۴ فراخوانی \lr{API} منحصربه‌فرد ساخته شد و توالی‌ها به اعداد صحیح توکنایز شدند\LTRfootnote{توکن پد = ۰}.
        \end{itemize}
\end{itemize}

\subsubsection{طراحی مدل \lr{MAGNET}}
مدل \lr{MAGNET} شامل سه ماژول تخصصی برای هر وجه، یک مکانیزم توجه پویا، یک طبقه‌بند باینری است. شکل \ref{fig:magnet_architecture} معماری کلی را نشان می‌دهد.

\begin{figure}[ht]
	\centering
    \includegraphics[width=0.9\textwidth]{images/magnet_architecture}
    \caption{معماری کلی مدل \lr{MAGNET}.}
    \label{fig:magnet_architecture}
\end{figure}

\paragraph{\lr{EnhancedTabTransformer} (ماژول جدولی)}
...
\lr{
\begin{algorithm}[h]
\caption{\lr{EnhancedTabTransformer Module Structure}}
\begin{algorithmic}[1]
...
\paragraph{\lr{GraphTransformer} (ماژول گرافی)}
این ماژول گراف‌های فراخوانی توابع را با استفاده از ترنسفورمر مبتنی بر \lr{GNN} پردازش می‌کند و از \texttt{TransformerConv} در \lr{PyTorch Geometric} بهره می‌برد:
\begin{itemize}
    \renewcommand{\labelitemi}{\lr{$\bullet$}}
    ...
    \]}
    که در آن \lr{\( W_{\text{node}} \in \mathbb{R}^{64 \times 64} \).}
    \item \textit{جاسازی یال}: ویژگی‌های یال در صورت وجود، به طور مشابه جاسازی می‌شوند.
    \item \textit{لایه‌ها}: چهار لایه با ۸ سر، اطلاعات را در سراسر گراف منتشر می‌کنند و سپس \lr{pooling} میانگین جهانی انجام می‌شود.
\end{itemize}
\lr{
\begin{algorithm}[h]
\caption{\lr{GraphTransformer Module Structure}}
\begin{algorithmic}[1]
...
\paragraph{\lr{SequenceTransformer} (ماژول ترتیبی)}
این ماژول توالی‌های فراخوانی \lr{API} را مدل‌سازی می‌کند:
\begin{itemize}
    \renewcommand{\labelitemi}{\lr{$\bullet$}}
    \item \textit{جاسازی}: توکن‌های \lr{API} با استفاده از واژه‌نامه‌ای با \lr{۲٬۱۳۴} فراخوانی منحصربه‌فرد، به بردارهای ۶۴ بعدی جاسازی می‌شوند.
...
\lr{
\begin{algorithm}[h]
\caption{\lr{SequenceTransformer Module Structure}}
\begin{algorithmic}[1]
...
\lr{
\begin{algorithm}[h]
\caption{\lr{Dynamic Attention Mechanism}}
\begin{algorithmic}[1]
...
\]}
سپس \lr{pooling} میانگین انجام می‌شود.
\lr{
\begin{algorithm}[h]
\caption{\lr{Modality Fusion}}
\begin{algorithmic}[1]
...
\paragraph{مدل نهایی \lr{MAGNET}}
مدل کامل، همه اجزا را ترکیب می‌کند:
\lr{
\begin{algorithm}[h]
\caption{\lr{Final MAGNET Model}}
\begin{algorithmic}[1]
...
مدل با استفاده از تابع زیان \lr{Binary Cross-Entropy} آموزش داده شد:
\lr{\[
\mathcal{L} = -\frac{1}{N} \sum_{i=1}^N [y_i \log(\hat{y}_i) + (1 - y_i) \log(1 - \hat{y}_i)],
\]}
با بهینه‌ساز \lr{Adam (\( \eta = ۰.۰۰۱ \)، \( \beta_1 = ۰.۹ \)، \( \beta_2 = ۰.۹۹۹ \)، {weight decay = ۰.۰۱})}. پارامترهای آموزش:
\begin{itemize}
    \renewcommand{\labelitemi}{\lr{$\bullet$}}
    \item اندازه دسته: ۳۲
    \item تعداد دوره‌ها: ۵۰ (با توقف زودهنگام، صبر = ۳)
    \item \lr{Dropout}: ۰.۲ در تمام لایه‌ها
\end{itemize}

\subsubsection{بهینه‌سازی ابرپارامترها}
برای تنظیم ابرپارامترها، از \lr{Optuna} با هدف بهینه‌سازی \lr{F1 Score} استفاده شد:
\begin{itemize}
    \renewcommand{\labelitemi}{\lr{$\bullet$}}
    \item \lr{\texttt{num\_heads}}: \{۴، ۸، ۱۶\}
    \item \lr{\texttt{num\_layers}}: \{۲، ۴، ۶\}
    \item \lr{\texttt{dropout}}: \{۰.۱، ۰.۲، ۰.۳\}
\end{itemize}
بهترین پیکربندی: \texttt{\lr{num\_heads} = 8} ، \texttt{\lr{num\_layers} = 4} ، \texttt{\lr{dropout} = 0.2}.

\subsubsection{ارزیابی مدل}
اعتبارسنجی متقاطع ۵-تایی انجام شد و معیارها به صورت زیر محاسبه شدند:
\begin{itemize}
    \renewcommand{\labelitemi}{\lr{$\bullet$}}
    \item دقت: \lr{\( \frac{\text{TP} + \text{TN}}{\text{TP} + \text{TN} + \text{FP} + \text{FN}} \)}
    \item \lr{F1 Score}: \lr{\( 2 \cdot \frac{\text{Precision} \cdot \text{Recall}}{\text{Precision} + \text{Recall}} \)}
    \item \lr{AUC}: مساحت زیر منحنی \lr{ROC}
\end{itemize}

نتایج به‌دست‌آمده عبارتند از: دقت \lr{97.24\% ± 0.5\%}، معیار \lr{F1 = 0.9823 ± 0.002}، و معیار \lr{AUC = 0.981 ± 0.003}. مقایسه با مدل‌های پایه \lr{SVM: 90.6\%}، \lr{CNN: 92.8\%}، \lr{LSTM: 91.5\%} نشان‌دهندهٔ برتری قابل‌توجه مدل \textbf{\lr{MAGNET}} است.

\subsection{جمع‌بندی روش پیشنهادی}
در این فصل، روش پیشنهادی \textbf{\lr{MAGNET}} برای شناسایی بدافزار اندروید به تفصیل شرح داده شد. این مدل با ادغام هوشمندانه داده‌های چندوجهی\lr{—}جدولی، گرافی و ترتیبی\lr{—}و با بهره‌گیری از قدرت معماری‌های نوین یادگیری عمیق نظیر ترنسفورمرها و شبکه‌های عصبی گرافی، گامی مهم در جهت افزایش دقت و پایداری سیستم‌های تشخیص بدافزار برداشته است. ماژول‌های تخصصی برای هر وجه داده‌ای، به همراه مکانیزم توجه پویا و لایه ادغام چندوجهی، به مدل امکان می‌دهند تا بازنمایی‌های غنی و جامعی از برنامه‌های اندرویدی استخراج کرده و الگوهای پیچیده مرتبط با رفتار مخرب را شناسایی کند.

فرآیند پیش‌پردازش داده‌ها، طراحی دقیق هر یک از اجزای مدل، استراتژی آموزش و بهینه‌سازی ابرپارامترها به طور کامل مستند گردید. ارزیابی‌های انجام‌شده بر روی مجموعه داده استاندارد \lr{DREBIN} و مقایسه با مدل‌های پایه، برتری قابل توجه مدل \lr{MAGNET} را در معیارهای کلیدی عملکرد نشان داد. این نتایج، پتانسیل رویکردهای چندوجهی و یادگیری عمیق پیشرفته را در مقابله با تهدیدات اندرویدی، به‌ویژه بدافزارهای پیچیده و نوظهور، تأیید می‌کند.

کارهای آتی می‌تواند در چند جهت گسترش یابد:
\begin{enumerate}
    \item \textbf{ارزیابی بر روی مجموعه داده‌های بزرگ‌تر و به‌روزتر}: آزمون مدل \lr{MAGNET} بر روی مجموعه داده‌های وسیع‌تر و جدیدتر مانند \lr{CICMalDroid} \cite{CICMalDroid} برای ارزیابی قابلیت تعمیم و مقیاس‌پذیری آن.
    ...
    \item \textbf{افزایش تفسیرپذیری (\lr{Explainability})}: توسعه روش‌هایی برای تفسیر تصمیمات مدل \lr{MAGNET}، به منظور درک بهتر اینکه کدام ویژگی‌ها یا الگوها در هر وجه بیشترین تأثیر را در شناسایی بدافزار دارند \cite{Marastoni2022}. این امر می‌تواند به تحلیلگران بدافزار در شناسایی تهدیدات کمک کند.
...
با این حال، مدل \lr{MAGNET} در شکل فعلی خود، یک چارچوب قدرتمند و انعطاف‌پذیر برای شناسایی بدافزار اندروید ارائه می‌دهد و می‌تواند به عنوان پایه‌ای برای تحقیقات آتی در این حوزه مورد استفاده قرار گیرد.
