...
\usepackage{listings}
\usepackage{xcolor}
\usepackage{bidi} % برای پشتیبانی از LTR در محیط‌های فارسی
\usepackage{hyperref}
\hypersetup{
    colorlinks=true,
    linkcolor=blue,
...
\begin{itemize}
    ...
    \item \textbf{سخت‌افزار}:
        \begin{itemize}
            \item \lr{GPU: NVIDIA RTX 3090} با ۲۴ گیگابایت VRAM و ۱۰٬۴۹۶ هسته CUDA، برای محاسبات موازی کارآمد.
...
\]
با بهینه‌ساز Adam (\( \eta = ۰.۰۰۱ \)، \( \beta_1 = ۰.۹ \)، \( \beta_2 = ۰.۹۹۹ \)، \lr{weight decay = ۰.۰۱}). پارامترهای آموزش:
\begin{itemize}
    \item اندازه دسته: ۳۲
...
نتایج به دست آمده عبارتند از: دقت \lr{97.24\%} (با انحراف معیار \lr{±0.5\%})، معیار \lr{F1} برابر با \lr{0.9823} (با انحراف معیار \lr{±0.002})، و معیار \lr{AUC} برابر با \lr{0.981} (با انحراف معیار \lr{±0.003}). مقایسه با مدل‌های پایه (\lr{SVM: 90.6\%, CNN: 92.8\%, LSTM: 91.5\%}) نشان‌دهنده برتری قابل توجه مدل \textbf{MAGNET} است.
