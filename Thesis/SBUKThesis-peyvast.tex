...
\section{پیوست \lr{A}: کدهای پیاده‌سازی مدل \lr{MAGNET}}

\subsection{کد معماری مدل \lr{MAGNET}}
این بخش کد اصلی معماری مدل \lr{MAGNET} را ارائه می‌دهد که در فصل 3 به‌صورت شبه‌کد توصیف شد. این کد با استفاده از \lr{PyTorch} پیاده‌سازی شده است.

\begin{LTR}
\begin{lstlisting}[language=Python, caption={کد معماری مدل MAGNET}, label={lst:magnet-architecture}, basicstyle=\scriptsize\ttfamily]
import torch
...
\subsection{کد بهینه‌سازی با \lr{PIRATES}}
این بخش قسمت اصلی کد بهینه‌سازی \lr{PIRATES} را نشان می‌دهد که برای تنظیم ابرپارامترها استفاده شد.

\begin{LTR}
\begin{lstlisting}[language=Python, caption={کد بهینه‌سازی با \lr{PIRATES}}, label={lst:pirates_optimization}, basicstyle=\scriptsize\ttfamily]
import numpy as np
...
\subsection{نمونه داده‌های خام \lr{DREBIN}}
این جدول نمونه‌ای از داده‌های خام دیتاست \lr{DREBIN} را نشان می‌دهد که برای آموزش مدل استفاده شد.

\begin{table}[ht]
    \centering
    \caption{نمونه‌ای از داده‌های خام دیتاست \lr{DREBIN}}
    \label{tab:raw_data_sample}
    \begin{tabular}{|l|c|c|c|}
        \hline
        \textbf{شناسه نمونه} & \textbf{تعداد مجوزها} & \textbf{فراخوانی‌های \lr{API}} & \textbf{برچسب} \\ 
        \hline
